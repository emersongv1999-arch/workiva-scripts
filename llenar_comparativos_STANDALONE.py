"""
llenar_comparativos_STANDALONE.py
==================================
Script para completar columnas de periodos en archivos "Base Notas"
de todas las sociedades de un periodo.

Las credenciales se guardan en config_comparativos.json (misma carpeta).
La primera vez que se ejecuta pide CLIENT_ID y CLIENT_SECRET.
WORKSPACE_ID es compartido para todos.
...
CONFIG_FILE  = Path(__file__).parent / "config_comparativos.json"

def _cargar_o_pedir_credenciales():
    if CONFIG_FILE.exists():
        try:
            ...
            if cid and cs:
                print(f"  Credenciales cargadas desde {CONFIG_FILE.name}")
                return cid, cs
        except Exception:
            pass

    print()
    print("  Primera vez: ingresa tus credenciales de Workiva.")
    print("  Se guardaran en config_comparativos.json para proximas ejecuciones.")
    print()
    cid = input("  CLIENT_ID     : ").strip()
    cs  = input("  CLIENT_SECRET : ").strip()
    if not cid or not cs:
        raise RuntimeError("Credenciales vacias — abortando.")
...
CLIENT_ID, CLIENT_SECRET = _cargar_o_pedir_credenciales()
