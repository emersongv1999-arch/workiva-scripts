...
class WorkivaClient:
    ...
    async def get(self, url: str, **kwargs) -> httpx.Response:
        """GET con reintentos: throttling (429), 5xx, errores de transporte y
        respuestas 200 con cuerpo vacío (vistas bajo carga sostenida)."""
        client = await self._ensure_client()
        ...
        return last  # agotados los reintentos: devolver la última respuesta
...
class FillComparativesInput(BaseModel):
    ...
    sheet_offset:   int = Field(
        default=0, ge=0,
        description="Índice de la primera hoja candidata a procesar en este lote (para paginar archivos grandes)"
    )
    max_sheets:     int = Field(
        default=20, ge=1, le=100,
        description="Máximo de hojas a procesar por llamada. Si el resultado trae has_more=true, vuelve a llamar con sheet_offset=next_offset"
    )
    exclude_sheets: list[str] = Field(
        default_factory=list,
        description="Nombres exactos de hojas adicionales a excluir en esta corrida"
    )
    apply_default_excludes: bool = Field(
        default=True,
        description="Si True (default) excluye las hojas auxiliares de soporte y los desgloses por sociedad (terminadas en CGEM/Edelmag). Ponlo en False para procesarlas también"
    )
    max_ejemplos:   int = Field(
        default=3, ge=1, le=1000,
        description="Máximo de ejemplos de diferencias reportados por columna en modo validación (dry_run)"
    )
    detalle_filas:  bool = Field(
        default=False,
        description="En modo validación, incluir el detalle fila por fila (etiqueta, valor destino, valor fuente, estado) de cada columna comparada"
    )
...
async def workiva_fill_comparatives(params: FillComparativesInput) -> str:
    ...
    Dos modos:
      - dry_run=True (validación, cualquier mes): no escribe; compara los
        valores de la fuente contra lo que ya tiene la columna comparativa del
        destino y reporta iguales/distintos con ejemplos por columna.
      - dry_run=False (escritura): aplica los valores. Solo permitido en
        archivos de cierre de marzo (mes 03, regla 11).

    Los archivos Base Notas tienen cientos de hojas, por lo que el proceso se
    pagina: cada llamada procesa hasta max_sheets hojas. Si el resultado trae
    has_more=true, vuelve a llamar con sheet_offset=next_offset hasta que
    has_more sea false.

    Por defecto se excluyen, además de las hojas técnicas, las auxiliares de
    soporte (VP, Sociedades, Traducción Notas, etc.) y los desgloses por
    sociedad (hojas terminadas en CGEM/Edelmag). Para procesarlas también,
    pasa apply_default_excludes=False.
    ...
    Args:
        ...
        params.sheet_offset:   Primera hoja candidata de este lote (default 0)
        params.max_sheets:     Hojas por lote (default 20)
        params.exclude_sheets: Hojas adicionales a excluir por nombre exacto
        params.apply_default_excludes: Excluir auxiliares y desgloses por
            sociedad (default True)

    Returns:
        JSON con resumen de hojas procesadas, columnas detectadas, operaciones
        realizadas (o simuladas en dry_run), y campos de paginación
        (has_more, next_offset, total_candidate_sheets).
    """
    # Hojas técnicas: se saltan siempre
    SKIP_SHEETS = {
    ...
    # Hojas auxiliares de soporte: excluidas por defecto (apply_default_excludes)
    AUX_SKIP_SHEETS = {
    ...
    # Desgloses por sociedad: nombre termina en CGEM / Edelmag (y typos vistos)
    ...
    try:
        ...
        bases: dict[str, str] = {}
        # Las fechas se ubican por la etiqueta de la columna B (col 1), no por
        # índice fijo de fila: la plantilla tiene filas vacías intercaladas y
        # el bloque puede moverse entre versiones del archivo.
        label_map = {
        ...
        # Parsear el nombre: [(CHN)/(LC) ]E{code}_{tipo}_{MM}-{YYYY}_{suffix}
        m = re.match(r"(\((?:CHN|LC)\)\s*)?(E\d+)_(IND|CONSO)_(\d{2})[-_](\d{4})_(.*)", target_name)
        if not m:
            report["warning"] = (
        ...
        # Solo mes 03 ESCRIBE comparativo de balance (regla 11). En dry_run la
        # herramienta funciona cualquier mes como modo validación: compara los
        # valores de la fuente contra lo que ya tiene el destino, sin escribir.
        if mm != "03" and not params.dry_run:
            report["message"] = (
                f"Solo se escribe el comparativo de balance en archivos de mes 03; "
                f"este archivo es del mes {mm}. Usa dry_run=True para comparar "
                f"saldos sin escribir."
            )
            return json.dumps(report, indent=2, ensure_ascii=False)

        # 4. Determinar hojas candidatas (presentes en destino y fuente).
        #    El detalle de hojas saltadas solo se reporta en el primer lote
        #    para no repetirlo en cada página; los desgloses por sociedad se
        #    reportan solo como conteo porque son ~150 hojas.
        ...
        # 5. Leer en paralelo (con límite de concurrencia) las hojas del lote
        ...
        comp_cols_by_name: dict[str, list[int]] = {}
        for sname in batch:
            comp_cols: list[int] = []
        ...
        src_cells_by_name = dict(zip(
            with_cols,
            await asyncio.gather(
                *(_read_lim(src_balance_id, src_sheets[s]) for s in with_cols)
            ),
        ))

        # 6. Por cada hoja con columnas comparativas, construir y aplicar valores
        for sname in with_cols:
            sid_t     = tgt_sheets[sname]
            tgt_cells = tgt_cells_by_name[sname]
            src_cells = src_cells_by_name[sname]

            sheet_report: dict[str, Any] = {
                "sheet": sname,
                "comp_cols": [_col_letter(c) for c in comp_cols_by_name[sname]],
                "cols_written": 0,
            }

            for dest_col in comp_cols_by_name[sname]:
                # Detectar columna fuente por offset
                src_col = dest_col - 2  # offset típico
                if src_col < 0:
                    continue

                # Valores de la fuente por fila. Para ESCRIBIR se saltan las
                # filas donde el destino tiene fórmula; para VALIDAR se
                # comparan todas (contra el valor calculado del destino).
                src_vals: list[Any] = []
                ...
                if not params.dry_run:
                    ...
                    # Modo validación: comparar la fuente contra lo que ya
                    # tiene la columna comparativa del destino.
                    ...
                    for i, v in enumerate(src_vals):
                        ...
                        # Celda vacía en destino se considera 0
                    ...
                    if params.detalle_filas:
                        # Contexto del encabezado (ej. "Corrientes"/"No corrientes"):
                        # el texto de grupo suele estar 2 columnas a la izquierda
                        # (sobre la columna del período actual). Se excluyen
                        # fechas, unidad M$ y el rótulo Agrupador.
                        textos: list[str] = []
                            ...
                            for c_h in (dest_col, dest_col - 1, dest_col - 2):
                                if 0 <= c_h < len(row_h):
                                    ...
                                    if (s and s not in ("M$", "Agrupador")
                                            ...
                                            and (not kw or kw not in s.lower())
                                            and s not in textos):
                                        textos.append(s)
                                        break
