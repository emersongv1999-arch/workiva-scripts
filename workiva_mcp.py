...
async def workiva_fill_comparatives(params: FillComparativesInput) -> str:
    ...
    try:
        ...
        bases: dict[str, str] = {}
        row_map = {
            12: ("current_end",    "prior_end"),
            13: ("eerr_start",     "prior_eerr_start"),
            14: ("eerr_end",       "prior_eerr_end"),
            15: ("quarter_start",  "prior_quarter_start"),
            16: ("prev_period_end","prior_prev_period_end"),
        }
        for row_idx, (key_c, key_p) in row_map.items():
            if row_idx >= len(bases_cells):
                continue
        ...
        # Parsear el nombre: E{code}_{tipo}_{MM}-{YYYY}_{suffix}
        m = re.match(r"(E\d+)_(IND|CONSO)_(\d{2})[-_](\d{4})_(.*)", target_name)
        if not m:
            report["warning"] = (
        ...
        if bases.get("prior_end"):
            ...
            for sep in ["-", "_"]:
                name = f"{code}_{tipo}_{mm_b}{sep}{yy_b}_{suffix}"
        ...
        for sname, sid_t in tgt_sheets.items():
            ...
            tgt_cells = await _read_sheet_cells(params.spreadsheet_id, sid_t)
            if not tgt_cells:
                continue
