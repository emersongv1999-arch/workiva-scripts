...
class App(tk.Tk):
    ...
    def _cmp_thread_procesar(self, seleccionados):
        try:
            ...
            for i, t in enumerate(seleccionados, 1):
                self._cmp_log_write(f"\n[{i}/{len(seleccionados)}] {t['name']}", "bold")
                try:
                    ...
                    n_sheets = len(preview.get("sheets_processed", []))
                    n_cols   = preview.get("total_cols_written", 0)
                    self._cmp_log_write(
                        f"  Preview: {n_sheets} hojas · {n_cols} columnas a escribir",
                        "muted")

                    if n_cols == 0:
                        self._cmp_log_write("  Sin columnas comparativas detectadas.", "warn")
                        total_err += 1
                        continue
                    ...
                    if "warning" in result:
                        ...
                        n_written = result.get("total_cols_written", 0)
                        sheets_ok = [s["sheet"] for s in result.get("sheets_processed", [])
                                     if s.get("cols_written", 0) > 0]
                        self._cmp_log_write(
                            f"  ✓ {n_written} columnas escritas en {len(sheets_ok)} hojas",
                            "ok")
                        total_ok += 1
