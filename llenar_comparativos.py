...
# ── EXCEPCIÓN DE PRUEBA ────────────────────────────────────────────────────────
# Cuando SOURCE_CODE_OVERRIDE está definido, al procesar el archivo indicado
# en OVERRIDE_TARGET, las fuentes se buscan usando SOURCE_CODE_OVERRIDE en
# lugar del código real del archivo destino.
# SOLO PARA PRUEBAS — dejar en None en producción.
OVERRIDE_TARGET      = ("E300", "03", "2026")   # (code, mm, yyyy) del archivo destino
SOURCE_CODE_OVERRIDE = "E200"                    # código a usar al buscar fuentes
# ──────────────────────────────────────────────────────────────────────────────
...
def find_source_files(parsed, bases, all_files):
    ...
    # Excepción de prueba: sustituir código de fuente si aplica
    if (SOURCE_CODE_OVERRIDE and
            (code, mm_curr, yyyy_curr) == OVERRIDE_TARGET):
        print(f"  [EXCEPCIÓN PRUEBA] Fuentes buscadas como {SOURCE_CODE_OVERRIDE} en vez de {code}")
        code = SOURCE_CODE_OVERRIDE
...
def build_write_values(tgt_cells, src_cells, dest_col, src_col):
    ...
    for i in range(len(tgt_cells)):
        ...
        vals.append(sv if isinstance(sv, (int, float)) and sv != 0 else None)
    if unmapped:
...
def main():
    _t0 = time.time()
    print("="*60)
    print("LLENADO DE COMPARATIVOS v2")
    print("="*60)
